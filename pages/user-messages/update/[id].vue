<template>
  <k-page>
    ...
    <k-toast position="center" :opened="opened.center">
      ...
      <div class="shrink">Message updated successfully!</div>
    </k-toast>
...
export default {
  ...
  methods: {
    reload() {
      ...
      axios
        .get(`https://cornerstone.test/api/resources/user-messages/` + this.$route.params.id + `/get`)
    ...
    back(){
      ...
      router.push(`/user-messages/` + this.$route.params.id);
    },
    update() {
      let data = {};
      ...
      axios
        .post(`https://cornerstone.test/api/resources/user-messages/`+ this.$route.params.id +`/update`, data)
        .then(response => (this.fields = response.data))
