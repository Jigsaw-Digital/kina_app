...
export default {
  ...
  methods: {
    reload() {
      ...
      axios
        .get(`https://cornerstone.test/api/resources/asset-addons/` + this.$route.params.id + `/get`)
    ...
    back(){
      ...
      router.push(`/asset-addons/` + this.$route.params.id);
    },
    update() {
      let data = {};
      ...
      axios
        .post(`https://cornerstone.test/api/resources/asset-addons/`+ this.$route.params.id +`/update`, data)
        .then(response => (this.fields = response.data))
